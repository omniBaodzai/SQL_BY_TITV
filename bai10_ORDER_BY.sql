...
GO
/* Ví dụ 04: Viết câu lệnh SQL lấy ra một sản phẩm có số lượng (Quantity) bán cao nhất
từ bảng [Order Details]. Không được sử dụng hàm MAX(). */
SELECT TOP 1 *
FROM [Order Details]
...
GO
/* Bài tập 02: Viết câu lệnh SQL liệt kê tên sản phẩm (ProductName), đơn giá (UnitPrice), số lượng sản phẩm
trong kho (UnitsInStock) và sắp xếp các sản phẩm theo thứ tự giảm dần của [UnitsInstock] từ bảng "Product". */
...
-- C. Thử thách. --
/* Thử thách 01: Viết câu lệnh SQL lấy ra cột Country và đặt tên thay thế là Quốc gia
từ bảng "Suppliers". Và tên Quốc gia được sắp xếp theo thứ tự tăng dần từ A - Z. */
...
GO
/* Thử thách 03: Viết câu lệnh SQL lấy ra 3 nhân có viên có tên (FirstName), ngày sinh (BirthDate) 
và ngày tuyển dụng (HireDate) khác nhau dựa theo tên (FirstName) từ A - Z từ bảng "Employees". */
