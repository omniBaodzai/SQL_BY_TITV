...
và đặt tên thay thế là "Tên" từ [bảng Employees]. */
...
FROM Employees;
GO
-- Ví dụ 03: Viết câu lệnh SQL lấy ra 15 dòng đầu tiên cho tất cả các cột từ [bảng Orders] và đặt tên thay thế cho [bảng Orders] là [o]. 
SELECT TOP 15 *
FROM Orders AS [o];
GO
-- Hoặc:
...
-- C. Thử thách. --
-- Thử thách 1: Viết câu lệnh SQL lấy ra cột "Country" và đặt tên thay thế là "Quốc gia" và chỉ lấy ra 15% số lượng dòng khác nhau từ [bảng Customer]. 
SELECT DISTINCT TOP 15 PERCENT Country AS "Quốc gia"
FROM Customers;
GO
/* Thử thách 2: Viết câu lệnh SQL lấy ra các mã thể loại (CategoryID) khác nhau và đặt tên thay thế là "Mã thể loại",
chỉ lấy ra 6 dòng đầu tiên và đổi tên [bảng Products] thành [p] rồi sử dụng tên thay thể đó để truy vấn cột cần lấy. */
