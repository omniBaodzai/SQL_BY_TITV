...
FROM Orders;
GO
/* Ví dụ 03: Viết câu lệnh SQL tính trung bình số lượng đơn đặt hàng (Quantity) của các sản phẩm
từ bảng "Orders Details". */
...
FROM Shippers;
GO
/* Thử thách 02: Viết câu lệnh SQL tính tổng số đơn đang đặt hàng (UnitsOnOrder) của các sản phẩm
từ bảng "Products". */
SELECT SUM(UnitsOnOrder) AS [TotalUnitsOnOrder]
FROM Products;
GO
/* Thử thách 03: Viết câu lệnh SQL tính trung bình số đơn được đặt hàng lại (ReorderLevel) của các sản phẩm
từ bảng "Products". */
